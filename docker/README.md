...
```bash
$ docker pull ageron/handson-mlp
```

**Note**: this is the CPU-only image. For GPU support, read the GPU section below.

Alternatively, you can build the image yourself. This will be slower, but it will ensure the image is up to date, with the latest libraries. For this, assuming you already downloaded this project into the directory `/path/to/project/handson-mlp`:

```bash
$ cd /path/to/project/handson-mlp/docker
$ docker-compose build
```

This will take quite a while, but is only required once.

After the process is finished you have an `ageron/handson-mlp:latest` image, that will be the base for your experiments. You can confirm that by running the following command:

```bash
$ docker images
REPOSITORY            TAG         IMAGE ID            CREATED             SIZE
ageron/handson-mlp    latest      3ebafebc604a        2 minutes ago       4.87GB
```

### Run the notebooks

Still assuming you already downloaded this project into the directory `/path/to/project/handson-mlp`, run the following commands to start the Jupyter server inside the container, which is named `handson-mlp`:
...
Run `make exec` (or `docker-compose exec handson-mlp bash`) while the server is running to run an additional `bash` shell inside the `handson-mlp` container. Now you're inside the environment prepared within the image.
...
- Replace `image: ageron/handson-mlp:latest` with `image: ageron/handson-mlp:latest-gpu`
...
```bash
$ docker pull ageron/handson-mlp:latest-gpu
```
...
```bash
$ cd /path/to/project/handson-mlp
$ docker run --name handson-mlp --gpus all -p 8888:8888 -p 6006:6006 --log-opt mode=non-blocking --log-opt max-buffer-size=50m -v `pwd`:/home/devel/handson-mlp ageron/handson-mlp:latest-gpu /opt/conda/envs/homlp/bin/jupyter notebook --ip='0.0.0.0' --port=8888 --no-browser
```
